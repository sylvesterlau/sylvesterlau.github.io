//读取 json
$.getJSON('./content.json', function (jsonObj) {
    var header = jsonObj.header;
    renderHeader(header);
    var article = jsonObj.article;
    article.forEach(function(item,index){addElementTo(item,index,"")});
})

//渲染 header
function renderHeader(header) {
    $(".header").append($('<h1></h1>').text(header.title));
    $(".header").append($('<p></p>').text(header.caption));
